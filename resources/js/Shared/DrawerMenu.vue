<template>
  <div v-if="isOpen" class="fixed inset-0 z-[70] overflow-hidden">
    <div class="absolute inset-0 bg-slate-950/58 backdrop-blur-sm transition-opacity" @click="close"></div>
    <div :class="[
      'fixed inset-y-0 right-0 w-full max-w-sm bg-slate-950/96 text-white shadow-[0_24px_80px_rgba(15,23,42,0.45)] transform transition-transform duration-300 ease-in-out backdrop-blur-xl border-l border-white/10',
      ...
      <nav class="mt-6 px-4">
        ...
        <div class="space-y-2">
          ...
          <a v-for="item in menuItems" :key="item.id" :href="item.url" @click="close" class="flex items-center space-x-3 rounded-2xl border border-transparent px-4 py-3.5 transition hover:border-white/10 hover:bg-white/8">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
